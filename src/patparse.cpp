...
bool pattern_parser(
    ...
    auto pattern_duplicate_nfa = [&](nfa_state_range copy_this_start,
                                     nfa_state_range copy_this_finish,
                                     nfa_state_range current_state,
                                     nfa_state_range &duplicate_finish) -> bool {
            ...
            if (!nfa_table[aux_state].fail) {
                ...
                if (nfa_table[aux_state].epsilon_out) {
                    ...
                    if (nfa_table[aux].ept.second_out == PATTERN_NULL)
                        ...
                        nfa_table[aux_state].ept.second_out =
                            nfa_table[aux].ept.second_out + offset;
                } else {
                    if (nfa_table[aux].epf.next_state == PATTERN_NULL)
                        nfa_table[aux_state].epf.next_state = nfa_table[aux].epf.next_state;
                    else
                        nfa_table[aux_state].epf.next_state =
                            nfa_table[aux].epf.next_state + offset;
    ...
    auto pattern_getnumb =
        [&](strlen_range &parse_count, int &number, char &ch, tpar_object in_string) -> bool {
    ...
    std::function<
        void(nfa_state_range, nfa_state_range &, strlen_range &, tpar_object, char &, int)>
        pattern_compound = [&](nfa_state_range first,
                               nfa_state_range &finish,
                               strlen_range &parse_count,
                               tpar_object in_string,
                               char &pat_ch,
                               int depth) {
            ...
            std::function<
                void(nfa_state_range, nfa_state_range &, strlen_range &, tpar_object, char &, int)>
                pattern_pattern = [&](nfa_state_range first,
                                      nfa_state_range &finish,
                                      strlen_range &parse_count,
                                      tpar_object in_string,
                                      char &pat_ch,
                    ...
                    auto pattern_range_delimgen = [&](nfa_state_range &range_patch,
                                                      int &range_start,
                                                      int &range_end,
                                                      bool &range_indefinite,
                                                      parameter_type &leading_param) {
                    ...
                    auto pattern_range_build = [&](int range_start,
                                                   int range_end,
                                                   nfa_state_range range_patch,
                                                   bool indefinite) {
                        int aux;
                        ...
                        for (aux = 2; aux <= range_start; ++aux) {
                            ...
                            dummy = pattern_duplicate_nfa(
                                begin_state, end_state, current_state, current_state
                            );
                        }
                    ...
                    while ((pat_ch != PATTERN_COMMA && pat_ch != PATTERN_RPAREN &&
                        ...
                        case TPD_PROMPT:
                            {
                                ...
                                if (pattern_getch(aux_count, aux_pat_ch, deref_span)) {
                                    ...
                                    if ((aux_count != deref_span.len) ||
                                        (deref_span.str[aux_count] == PATTERN_COMMA)) {
                        ...
                        case '^':
                                ...
                                if (pat_ch == TPD_SPAN || pat_ch == TPD_PROMPT) {
                                    ...
                                    if (aux >= 2) { // one $ plus one "
                                        if (deref_tpar.str[aux - 1] == delimiter) {
                                            ...
                                            no_dereference = true; // ok we drop out and let the
                                                                   // normal processing take it
                                        }
                                    } else {
                                ...
                                if ((pat_ch == PATTERN_DEFINE_SET_U) ||
                                    ...
                                    while (aux <= deref_span.len) {
                                        ...
                                        if (aux + 2 <= deref_span.len) {
                                            if ((deref_span.str[aux] == '.') &&
                                                (deref_span.str[aux + 1] == '.')) {
                                    ...
                                    for (aux_count = 1; aux_count <= deref_span.len; ++aux_count)
                                        pattern_definition.strng[temporary + aux_count] =
                                            deref_span.str[aux_count];
                                    pattern_definition.length =
                                        temporary + deref_span.len + 1; // plus room for null
                                    ...
                                    if (POSITIONALS.contains(pat_ch)) {
                                        ...
                                        case 'S':
                                            nfa_table[current_state].epf.accept_set = SPACE_SET;
                                            break;
                                        case 'C':
                                            nfa_table[current_state].epf.accept_set = PRINTABLE_SET;
                                            break;
                                        case 'A':
                                            nfa_table[current_state].epf.accept_set = ALPHA_SET;
                                            break;
                                        case 'L':
                                            nfa_table[current_state].epf.accept_set = LOWER_SET;
                                            break;
                                        case 'U':
                                            nfa_table[current_state].epf.accept_set = UPPER_SET;
                                            break;
                                        case 'N':
                                            nfa_table[current_state].epf.accept_set = NUMERIC_SET;
                                            break;
                                        case 'P':
                                            nfa_table[current_state].epf.accept_set =
                                                PUNCTUATION_SET;
                            ...
                            if (leading_param == parameter_type::pattern_range) {
                                ...
                                nfa_table[current_state].ept.first_out = PATTERN_NULL;
                                pattern_range_build(
                                    range_start, range_end, range_patch, range_indefinite
                                );
                            }
                        }
            ...
            pattern_pattern(
                nfa_table[current_e_start].ept.first_out,
                finish,
                parse_count,
                in_string,
                pat_ch,
            ...
            if (pat_ch == PATTERN_BAR) {
                if (pattern_getch(parse_count, pat_ch, in_string)) {
                    ...
                    pattern_compound(
                        nfa_table[current_e_start].ept.second_out,
                        finish,
                        parse_count,
                        in_string,
                        pat_ch,
    ...
    try {
        ...
        if (pattern_getch(parse_count, pat_ch, pattern)) {
            pattern_compound(
                first_pattern_start, first_pattern_end, parse_count, pattern, pat_ch, 1
            ); // 1st
            if (pat_ch == PATTERN_COMMA) {
                ...
                if (pattern_getch(parse_count, pat_ch, pattern)) {
                    ...
                    pattern_compound(
                        left_context_end, middle_context_end, parse_count, pattern, pat_ch, 1
                    ); // 2nd
                } else {
                    // null middle
            ...
            if (pat_ch == PATTERN_COMMA) {
                ...
                if (pattern_getch(parse_count, pat_ch, pattern)) {
                    pattern_compound(
                        middle_context_end, pattern_final_state, parse_count, pattern, pat_ch, 1
                    ); // 3rd
                } else {
                    // null right context
