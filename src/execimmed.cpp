...
namespace {
    ...
    inline constexpr std::string_view DEFAULT_SPAN_NAME("L. Wittgenstein und Sohn.");
...
void execute_immed() {
    ...
    case ludwig_mode_type::ludwig_screen:
        {
                ...
                if (edit_mode == mode_type::mode_command) {
                    ...
                    while (true) {
                        ...
                        if (edit_mode == mode_type::mode_overtype)
                            cmd_success =
                                text_overtype(false, 1, input_buf, input_len, current_frame->dot);
                        else
                            cmd_success =
                                text_insert(false, 1, input_buf, input_len, current_frame->dot);
                        if (cmd_success) {
                            ...
                            if (!mark_create(
                                    current_frame->dot->line,
                                    current_frame->dot->col - input_len,
                        ...
                        if (current_frame->dot->col != current_frame->margin_right + 1) {
                            ...
                            // AT THE RIGHT MARGIN.
                            if (current_frame->options.contains(
                                    frame_options_elts::opt_auto_wrap
                                )) {
                                ...
                                if (PRINTABLE_SET.test(key) && key != command_introducer) {
                                    ...
                                    if (key != ' ') {
                                        while ((*current_frame->dot->line->str)[col_1] != ' ' &&
                                               col_1 > current_frame->margin_left)
                                            col_1 -= 1;
                                        col_range col_2 = col_1;
                                        while ((*current_frame->dot->line->str)[col_2] == ' ' &&
                                               col_2 > current_frame->margin_left)
                                            col_2 -= 1;
                                        if (col_2 ==
                                            current_frame->margin_left) // Line has only one word
                                            col_1 = current_frame
                                                        ->margin_right; // Split at right margin
                                        vdu_take_back_key(key);
                                    }
                                    current_frame->dot->col += 1;
                                    cmd_success = text_split_line(
                                        current_frame->dot, 0, current_frame->marks[MARK_EQUALS]
                                    );
            ...
            l9:;
                if (tt_controlc) {
                    ...
                    vdu_beep();  // Complain.
                    vdu_flush(); // Make sure they hear the complaint.
                } else {
                    jammed = false;
                }
                exit_abort = false;
    ...
    case ludwig_mode_type::ludwig_batch:
        {
            // with cmd_span^ do
            cmd_span.mark_one = std::make_shared<mark_object>();
            cmd_span.mark_one->line = nullptr;
            cmd_span.mark_one->col = 1;
            cmd_span.mark_two = std::make_shared<mark_object>();
            ...
            if (file_create_open(cmd_fnm, parse_type::parse_stdin, cmd_file, dummy_fptr)) {
                do {
                    ...
                    if (file_read(
                            ...
                            cmd_count,
                            true,
                            cmd_span.mark_one->line,
                            cmd_span.mark_two->line,
                            i
                        )) {
                        if (cmd_span.mark_one->line != nullptr) {
