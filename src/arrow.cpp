...
bool arrow_command(commands command, leadparam rept, int count, bool from_span) {
    ...
    line_range eop_line_nr =
        current_frame->last_group->first_line_nr + current_frame->last_group->last_line->offset_nr;
    ...
    while (true) {
            ...
            case commands::cmd_home:
                {
                    ...
                    if (current_frame == scr_frame) {
                        if (!mark_create(
                                scr_top_line, current_frame->scr_offset + 1, current_frame->dot
                            ))
                            goto l9;
                    }
                }
        ...
        if (from_span)
            goto l9;
        screen_fixup();
        if (!cmd_valid || ((command == commands::cmd_down) && (rept != leadparam::pindef) &&
...
l9:;
    if (tt_controlc) {
        ...
        if (cmd_status) {
            ...
            if ((command == commands::cmd_down) && (rept != leadparam::pindef) &&
                (current_frame->dot->line->flink == nullptr))
                cmd_status = false;
        }
    }
