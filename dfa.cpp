...
bool pattern_dfa_table_initialize(dfa_table_ptr &pattern_ptr, const pattern_def_type &pattern_definition) {
    if (pattern_ptr != nullptr) {
        //with pattern_ptr^ do
        for (int count = 0; count <= pattern_ptr->dfa_states_used; ++count) {
            //with dfa_table[count] do
...
bool pattern_dfa_convert(nfa_table_type &nfa_table,
    ...
    while (unmarked_states(current_state)) {
        if (tt_controlc) {   // a reasonable place for it, gets tested once per state, = about 0.03 seconds actual cp
        ...
        if ((partition_ptr != nullptr) && (partition_ptr->flink != nullptr)) {
            ...
            while (current_partition_ptr != nullptr) {
                ...
                while (aux_partition_ptr != nullptr) {
                    if (current_partition_ptr->accept_set_partition == aux_partition_ptr->accept_set_partition) {
                        // merge entrys
                        ...
                        aux_state_ptr->next_elt = aux_partition_ptr->nfa_transition_list.equiv_list; // patch list on
                        // remove aux entry
                        ...
                        intersection_set = current_partition_ptr->accept_set_partition
                                           .set_intersection(aux_partition_ptr->accept_set_partition);
                        if (!intersection_set.empty()) { // preeety worthless if []
                            if (intersection_set == current_partition_ptr->accept_set_partition) {
                                current_partition_ptr->nfa_transition_list.equiv_list = transition_list_append(
                            ...
                            } else if (intersection_set == aux_partition_ptr->accept_set_partition) {
                                aux_partition_ptr->nfa_transition_list.equiv_list = transition_list_append(
                                    aux_partition_ptr->nfa_transition_list.equiv_list,
                                    current_partition_ptr->nfa_transition_list.equiv_list);
                                current_partition_ptr->accept_set_partition.remove(intersection_set);
                            } else {
                                ...
                                insert_partition->nfa_transition_list.equiv_list = transition_list_merge(
                                    current_partition_ptr->nfa_transition_list.equiv_list,
                                    aux_partition_ptr->nfa_transition_list.equiv_list);
                                current_partition_ptr->accept_set_partition.remove(intersection_set);
                                aux_partition_ptr->accept_set_partition.remove(intersection_set);
        ...
        while (partition_ptr != nullptr) {
            ...
            if (!epsilon_closures(aux_partition_ptr->nfa_transition_list, transfer_state))
                return false;
            if (!pattern_add_dfa(transfer_state, aux_partition_ptr->accept_set_partition, current_state))
                return false;
    ...
    while (incoming_tran_ptr != nullptr) { // find all transitions out of start
        ...
        if ((incoming_tran_ptr->accept_next_state != PATTERN_DFA_KILL) &&
            ...
            dfa_state_type &dtans(dfa_table_pointer->dfa_table[incoming_tran_ptr->accept_next_state]);
            ...
            while ((kill_tran_ptr != nullptr) &&
                ...
                if (kill_tran_ptr != nullptr) {
                    aux_transition_set =
                        incoming_tran_ptr->transition_accept_set.set_intersection(kill_tran_ptr->transition_accept_set);
                    if (!aux_transition_set.empty()) {
                        ...
                        kill_tran_ptr->transition_accept_set.remove(aux_tran_ptr->transition_accept_set);
                ...
                while (aux_tran_ptr_2 != nullptr) {
                    ...
                    if (state > aux_count_2) {
                        ...
                        while ((aux_tran_ptr != nullptr) && !found) {
                            if (aux_tran_ptr->accept_next_state == state) // has a transition to itself
                                found = true;
                            else
                                aux_tran_ptr = aux_tran_ptr->next_transition;
                        }
                        if (found) {
                            dfa_table_pointer->dfa_table[state].left_context_check = true; // assume the worst
                            for (aux_count = middle_context_start; aux_count <= right_context_start; ++aux_count) {
                                ...
                                if (nfa_table[aux_count].indefinite && aux_set.contains(aux_count) &&
                                    dfa_table_pointer->dfa_table[state].nfa_attributes.equiv_set.contains(aux_count))
